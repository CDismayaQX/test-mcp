...
_TOOL_NAME = "order_split_lookup"
_TOOL_VERSION = "1.0"
_INTERNAL_BRAND_ID = "prolook"
_INTERNAL_BRAND_NAME = "PROLOOK"


async def _handle_order_split_lookup(
    order_id: str,
    brand_id: str | None,
    client: IOrderClient,
) -> ToolResult:
    """Core logic for order split lookup — directly testable without FastMCP."""
    ctx = BrandContext(
...
def register_order_split_lookup(mcp: fastmcp.FastMCP, client: IOrderClient) -> None:
    @mcp.tool()
    async def order_split_lookup(order_id: str, brand_id: str | None = None) -> dict[str, Any]:
        """Look up a split order with its line items by ID. Optionally scoped to a brand."""
        started = time.monotonic()
        result = await _handle_order_split_lookup(order_id, brand_id, client)
        ...
        await write_audit_event(
            AuditEvent(
                ...
                input_args={"order_id": order_id, "brand_id": brand_id},
