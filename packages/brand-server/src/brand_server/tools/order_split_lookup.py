...
def register_order_split_lookup(mcp: fastmcp.FastMCP, client: IOrderClient) -> None:
    @mcp.tool()
    async def order_split_lookup(order_id: str) -> dict[str, Any]:
        """Look up a split order with its line items by ID. Scoped to the authenticated brand."""
        started = time.monotonic()
        brand_ctx = get_brand_context()
        result = await _handle_order_split_lookup(order_id, client, brand_ctx)
